...
| `shapedump.js` + `shapecmp.py` + `runshape.sh` | **Beweis für einen Refactor ohne sichtbare Änderung**: berechnete Form-Styles (Schatten, Radien, Hintergrundbilder, Transform, Rand) jedes Elements samt Pseudo-Elementen, dazu Hover und Press, über Routen × hell/dunkel × Desktop/Phone — einmal gegen master, einmal gegen den Branch, beide hinter `previewproxy.js`. `shapecmp.py` muss `0 differences` melden; den Proxy-Port in `url()`s vorher normalisieren. Gebaut für #198 (~25 min für 10 Routen) |
