...
- ⚠️ **`--delete-branch` löscht „nach dem Merge" — und mit `--auto` gibt es dieses „nach" nicht.**
  ...
  und kehrt sofort zurück, gemergt wird Minuten später auf GitHub, ohne laufenden Client. `gh`
  räumt dort also **nichts** auf, weder lokal noch remote — beobachtet an #165 und #166, beide
  Branches standen nach dem Merge noch mit lebendem Remote da.
...
fällt durch jeden Sweep (so entstanden 76 der 183). ⚠️ Beim `--auto`-Merge kommt die
`[gone]`-Markierung aber nicht von diesem Flag, sondern von der Repo-Einstellung oben — deshalb
nach dem Merge einmal gegenprüfen, statt es anzunehmen:
